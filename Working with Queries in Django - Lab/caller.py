...
def find_authors_nationalities():
    searched_authors = Author.objects.filter(nationality__isnull=False)
    return '\n'.join([f"{author.first_name} {author.last_name} is {author.nationality}" for author in searched_authors])

# Task 3


def order_books_by_year():
    books_by_publication_year = [f'{book.publication_year} year: {book.title} by {book.author}'
                                 for book in Book.objects.all().order_by('publication_year', 'title')]
    return '\n'.join(books_by_publication_year)
...
def filter_authors_by_nationalities(nationality):
    ...
    output = [author.biography if author.biography else f'{author.first_name} {author.last_name}' for author in authors]
...
def filter_authors_by_birth_year(first_year, second_year):
    years = [x for x in range(first_year, second_year + 1)]
